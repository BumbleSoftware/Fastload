...
import com.abdelaziz.fastload.config.screen.BuildingTerrainScreen;
import com.abdelaziz.fastload.init.Fastload;
import com.abdelaziz.fastload.util.TickTimer;
...
import static com.abdelaziz.fastload.client.FLClientEvents.*;
...
public final class FLClientHandler {
    ...
    private static void registerEvents() {
        ...
        SET_SCREEN_EVENT.register(1, (eventContext, abstractEvent, closer, eventArgs) -> {
            if (CLIENT_TIMER.isReady() && eventContext.screen() instanceof GameMenuScreen && !client.isWindowFocused()) {
        ...
        PAUSE_MENU_EVENT.register(1, (eventContext, abstractEvent, closer, eventArgs) -> {
            if (justLoaded) {
                if (client.isWindowFocused()) justLoaded = false;
                else {
                    justLoaded = false;
                    eventContext.ci().cancel();
